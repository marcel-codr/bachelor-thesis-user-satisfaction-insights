...
ChatGPT reached 1 million users within 5 days after its launch in 2022 which was only exceeded by Threads in 2023 \parencite{Brandt2023}. Today ChatGPT provides benefits for 800 million weekly users \parencite{Huber2025} and thus has become an established Generative AI (GenAI) tool. GenAI bares high potential not only for the private sector but also for the modern business world. Projections underscore this, suggesting that GenAI could increase Germany's GDP by up to \EUR{585} billion by 2040 \parencite[p.~12]{McKinsey2023}. Furthermore for enterprise applications, \textbf{Retrieval-Augmented Generation (RAG)} has emerged as a particularly suitable technology by connecting large language models (LLMs) to company-specific proprietary knowledge bases  \parencite[p.~9459]{Lewis2020}. Connecting and bounding LLMs to proprietary knowledge bases not only enables them to access up-to-date company data \parencite{GoogleCloudRAG} but also enhances transparency and reduces the likelihood of hallucinations, as the company data serves as the ground truth  \parencite[p.~9469]{Lewis2020}.
...
\begin{longtable}{l >{\RaggedRight}p{3cm} >{\RaggedRight}p{3.5cm} >{\RaggedRight}p{3.5cm} >{\RaggedRight}p{3.5cm}}
    ...
    1 & Entwickler & \textbf{5}: Es ist alles sehr klar und übersichtlich gestaltet. Ich finde es sehr gut designed. Ich finde mich gut zurecht. & \textbf{5}: Was ich cool fänd, ist wenn man die Zeitspanne per maus im Graphen auswählen kann. Wie so ein sliding window. Vielleicht könnte man auch für uns release dates/changes an der Pipeline in dem Chart markieren dann könnte man Qualität zu Änderungen an der Pipeline nachvollziehen. & \textbf{5}: Ich denke es ist sehr wichtig die Qualität der Antworten zu überwachen. \\
    \midrule
    2 & Product Owner & \textbf{3}: Mehr Legenden wären hilfreich, z. B. Score liegt zwischen 0 und 1, was genau bedeuten die Kategorien wie Treue. & \textbf{5}: Alle Werte sind gut ablesbar. Verbesserung: Siehe oben, mehr Beschreibungen für Nutzerinfos. & \textbf{5}: Schafft mehr Transparenz für den Nutzer. \\
    \midrule
    3 & Customer Success Manager & \textbf{4}: Ich finde die Erklärung oben sehr gut! Die Skalen von 0-1 finde ich per se klar, aber die Bezeichnung der Skalen würde dennoch helfen. & \textbf{5}: Tranparenz hinsichtlich der Erklärung, wie sich die einzelnen Werte berechnen (non-technisch und nicht zu ausschweifend). Verbesserung: Skalen bezeichnen, y-Achse insbesondere, Vergleichbarkeit schaffen. & \textbf{5} \\
    \midrule
    4 & Manager & \textbf{4}: Ich überlege, ob es einen Zusammenhang zwischen den 4 kategorien gibt. Ist nicht eine hohe Treue, von hoher Genauigkeit und vollständigkeit abhängig? Falls ja, wäre das gut, dem Nutzer zu erklären, oder es vllt sogar wie einen Informations funnel drop-off darzustellen. & \textbf{4}: Ich denke nochmal auf die abhängigkeit der kategorien eingehen. & \textbf{5} \\
    \midrule
    5 & Entwickler/TPO & \textbf{3}: Man muss sich zumindest mal die Beschreibung gut durchlesen, die 4 Metriken sind jetzt glaube ich gerade für Laien (irgendwelche Gabelstaplerverkäufer zB) bestimmt nicht bekannt. & \textbf{4}: Gibt einen Überblick, aber da es durch LLMs erstellt wird, kann es glaube ich relativ random sein. Bzw ist die Frage so, was an Kontext alles dazu kommt. Also meine Vermutung wäre zB, dass ein menschlicher Experte wahrscheinlich oft anders scoren würde. Verbesserung: Vielleicht noch die Möglichkeit die 4 Kategorien zu gewichten und daraus einen "Superscore" basteln. & \textbf{2}: Glaube es ist besser als nichts, aber würde jetzt nur rein aus Intuition so vermuten, dass es wahrscheinlich nicht super akkurat ist. \\
    \midrule
    6 & Developer & \textbf{5}: Nutzung eindeutig. & \textbf{3}: LLM bewertet sich im Grunde selbst indem es seine eigene Antwortqualität benotet. Ist zwar einfach + vollautomatisch umsetzbar aber dafür sehr fehleranfällig. Verbesserung: Komplexere Evaluationspipeline bauen bspw. mit Inkorporation von Websuche + Self-reflection - sorgt dann halt auch wieder für deutlich höhere Kosten. & \textbf{4}: Automatische Qualitätsevaluation ist immer sinnvoll. \\
    \midrule
    7 & Software Entwickler & \textbf{3}: Ist villeicht nicht intuitiv genug für einen Nutzer. & \textbf{5}: Immerhin ist die funktion nützlich und es erfüllt die Funktionalität. Verbesserung: Villeicht es intuitiver ohne graphen, sonder nur mit einem einzelnen score. & \textbf{5}: Die Funktion ist nützlich, es erstellt eine neue nützliche information der Antwortqualität zum verbessern des Produkts. \\
    \midrule
    8 & Softwareentwickler & \textbf{5}: bekannte Diagrammart, Farben stimmen überein, Filter sind intuitiv verständlich. & \textbf{4}: (A) kann gut erkannt werden ob der Trend nach oben geht. (B) könnte schwierig werden, da der Nutzer ggf. nicht weiß, ab welchem Wert keine weitere Maßnahme notwendig ist. Verbesserung: verschiedene Arten von Punkten (Kreis, Quadrat, etc.), um Linien nicht nur anhand der Farbe unterscheiden zu können. Für (B) eine Art Threshold (vline) anzeigen. & \textbf{5} \\
    \midrule
    9 & Softwareentwickler & \textbf{3}: Beispiele in einem Help Tooltip wären sinnvoll um die einzelnen Metriken nochmal genauer zu erläutern. & \textbf{5}: Verbesserung: Prüfen wie gut die Evaluierungen durch das LLM sind und ob sie deswegen wirklichen Mehrwert bieten. Vielleicht noch einen Disclaimer hinpacken, dass diese Werte auch alle KI-generiert sind. & \textbf{4} \\
    \midrule
    10 & Developer & \textbf{5} & \textbf{5}: Verbesserung: Titel des Diagramms finde ich zu generisch. Erklärung was Maßnahmen sein könnten, um Werte zu verbessern? Information wie genau die Werte ermittelt wurden? Ist 1 überhaupt realistisch erreichbar? Was wäre ein "guter" Wert? & \textbf{5} \\
\end{longtable}
...
\begin{longtable}{l >{\RaggedRight}p{3cm} >{\RaggedRight}p{3.5cm} >{\RaggedRight}p{3.5cm} >{\RaggedRight}p{3.5cm}}
    ...
    1 & Entwickler & \textbf{5} & \textbf{5}: Es wäre cool, wenn man noch fein granularer z.B. in einer Tabelle pro Use-Case die Metriken auf einer Dokumenten Ebene sehen könnte. & \textbf{5}: Absolut, ala garbage in garbage out. \\
    \midrule
    2 & Product Owner & \textbf{3}: Siehe vorhergehender Reiter (mehr Legenden, Beschreibungen). & \textbf{5} & \textbf{5} \\
    \midrule
    3 & Customer Success Manager & \textbf{3}: Beispiele zur Bewertung angeben, damit das Feature für non-technicals verständlich wird. & \textbf{3} & \textbf{2}: Dokumentenqulität sehr wichtig, die generelle Zahl hilft jedoch im Zweifel weniger als die Analyse einzelner Themenbereiche oder gar Dokumente. Vll kann man zwischen Sources unterscheiden? Dann wäre pro Use Case ein deutlicher Mehrwert gewonnen. \\
    \midrule
    4 & Manager & \textbf{4}: Die werte befinden sich teil über 1. anders als in der beschreibung. & \textbf{4}: ist es besser eine höhere anzahl verwendeter Dokumente zu haben, oder eine niedrigere? & \textbf{5} \\
    \midrule
    5 & Entwickler/TPO & \textbf{5}: Glaube "Relevanz" und "Anzahl gefundener Dokumente" sind eher Begriffe, die man auf Anhieb versteht. & \textbf{4}: Verbesserung: Eventuell noch die Dokumente aus dem Retriever auch mit reinnehmen? (optional). Manchmal ist ja doch auch der Reranker einfach dumm, bzw gibt er auch oft ganz schön andere Scores als der Retriever. & \textbf{5}: Ist einfach zu verstehen und man kann denke ich es gut nutzen, um die Datenlage zu analysieren und daraus Schritte abzuleiten. \\
    \midrule
    6 & Developer & \textbf{5}: Nutzung eindeutig. & \textbf{3}: Hier sind die Scores am Ende halt auch wieder das Ergebnis von je einem LLM Call. Das Problem ist halt hier dass das LLM den Retriever bewertet und es uns nur mitteilt dass die Dokumente gut/schlecht sind aber keinen tieferen Insight liefert WARUM. & \textbf{4} \\
    \midrule
    7 & Software Entwickler & \textbf{3}: Wie schon vorher begründet, villeicht sind die Graphen zu komplex für einen Endnutzer, zu viele metriken können auch verwirrend sein. & \textbf{5}: Mir scheint es die gewünschte funktionalität zu erfüllen. Verbesserung: Dumb it down for the end user. & \textbf{5}: Sieht gut aus, ist eine nützliche information, es kann benutzt werden für die verbesserung und entwicklung von wingman. \\
    \midrule
