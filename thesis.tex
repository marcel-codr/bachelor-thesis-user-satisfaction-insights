...
ChatGPT reached 1 million users within 5 days after its launch in 2022 and became the fastest-growing application in history. This rapid growth highlights the huge potential of Generative AI (GenAI), not only in the private sector but also within the modern business world, where it is seen as a significant driver of corporate growth. Projections underscore this, suggesting that GenAI could increase Germany's GDP by up to \EUR{585} billion by 2040. For enterprise applications, \textbf{Retrieval-Augmented Generation (RAG)} has emerged as a particularly suitable technology, as it connects Large Language Models (LLMs) to company-specific proprietary knowledge bases. Connecting and bounding the LLM to the proprietary knowledge bases not only lets it access up-to-date company data but also improves transparency and avoids hallucinations because now the company data functions as the ground truth.

However, the success of enterprise RAG applications is not guaranteed. These systems can be unstable, leading to low-quality results. Common failure points include the system failing to provide an answer even when the information is available, generating a false answer by misinterpreting context, or returning an incomplete response. As organizations invest in and deploy these powerful but complex systems, a critical management challenge emerges: How can the success of a GenAI application be reliably measured and monitored over time? Without systematic evaluation, system administrators lack the data-driven insights needed to identify weaknesses, justify improvements, and ensure that the system delivers a positive return on investment.

This thesis addresses this management challenge directly. It documents the design, development, and evaluation of an analytics dashboard created to automatically measure the success of an enterprise GenAI application. Grounded in the theoretical framework of the DeLone and McLean IS Success Model, the artifact provides system administrators with insights into the three ISSM quality metrics - information quality, system quality and serive quality. Following a Design Science Research (DSR) methodology, the features of this dashboard were conceptualized and evaluated in close collaboration with the development team of an existing RAG system, ensuring both academic and practical relevance.
%---------------------------------------------------------------------%
