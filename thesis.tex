...
\begin{itemize}
    ...
    \item \textbf{Feedback:} Users can provide individual feedback on the quality of responses.
...
First, this chapter presents the results of the developer workshops, namely the resulting concepts and their contribution to the GenAI success evaluation framework, as well as the prioritization and feedback by the developer team. Then, it describes the software features implemented during this thesis from a usage perspective and technically and presents the evaluation by the development team.
...
The objective of this feature is to empower Use Case administrators to identify performance trends and retrospectively evaluate the impact of their own configuration decisions on response times. Furthermore, it allows them to better assess how different system factors influence system latency. These insights can, in turn, be used to train employees on the effective use of the system, for example, by providing better guidance on the performance implications of choosing a particular language model.

During the workshop, the development team assigned a medium priority to this feature. In their feedback, they noted that while the feature would be straightforward to implement, they suspected that overall response times would not vary significantly. Therefore, they hypothesized that this analysis would likely not yield major conclusions.

\subsubsection{Error Rate Monitoring}
A second concept proposed to evaluate system quality focuses on monitoring error rates to assess system stability and reliability. The core metrics to be captured are the absolute count and the percentage of user queries that result in a system error. For a more granular analysis, these error metrics can be combined with system factors (such as the selected LLM, or the activation state of Query Splitting and Reformulation) and with different, pre-defined error types.

The visalization propsed for these data is a time-series diagram again. The x-axis would represent time, while the y-axis would display the amount or percentage of system errors. The interface is designed to be interactive, featuring filtering options for both system factors and error types. This allows for detailed analysis, such as comparing the error rates of different language models or analyzing the impact of query splitting and query reformulation on error rates.
...
A crucial concept for evaluating the system is the direct measurement of its output quality. This feature focuses on assessing both the relevance of the retrieved documents and the quality of the final, generated response. The main goal is to establish metrics for how well the system performs to ensure users receive helpful and accurate answers.

The discussion highlighted that there is no single, perfect way to measure quality automatically. Therefore, the team proposed different approaches that could suite the quality measurement:
...
The results, such as the percentage of "good" and "bad" responses, would be displayed in a time-series chart. This would also include filters for system factors (like the model selection or Query Splitting) to see how they affect quality.

The development team unanimously assigned this feature a \textbf{high priority}, describing it as one of the most critical factors for the project's success. The team emphasized that guaranteeing high-quality results is essential for user retention; if users do not get good answers, they will stop using the system. While they acknowledged the challenge of finding a truly reliable automated quality metric, they agreed that developing good proxies for quality was vital.
\subsubsection{Combined Quality and Topic/Task Map}
Another concept presented in the workshop was a combination of the previous ideas into a single, powerful visualization. This "Combined Quality and Topic/Task Map" was designed to connect the system's performance quality directly with the specific topics or task types being handled. The goal is to identify patterns and understand if the system's quality varies for different kinds of user queries.

This feature uses the bubble plot from the Topic or Task Type maps as its foundation. The positioning of the bubbles (semantic similarity) and their color (topic clusters) remain the same, as does the sliding time window for dynamic analysis. The crucial change lies in the meaning of the bubble size. Instead of representing the frequency of a topic, the size of each bubble now represents the average retrieval or response quality for that topic. For example, a large bubble could signify a topic with consistently high-quality answers, while a smaller bubble might indicate a topic where the system often performs poorly.
...
The feedback for this feature was positive, with priority ratings ranging from \textbf{medium} to \textbf{high}. The team agreed that this analysis could provide "good insights" into user behavior. Understanding which tasks are most common for high-interest topics could help administrators better structure content or create more relevant "Quick Actions" to improve the user experience for those specific topics.
\subsection{Measurements of the service quality}
\subsubsection{User Interactivity Tracking}
To evaluate the service quality of the application, a concept for tracking user interactivity was discussed. The idea is to monitor and count a wide range of specific user events that happen during a session. This approach looks beyond the initial query to understand the user's complete interaction, including actions like copying an answer, viewing a source document, or providing direct feedback.

The data would be visualized in a time-series diagram, showing the frequency of these different events over a selected period. A key aspect of this concept is to also provide context or an interpretation of what each event could mean. For example, a high number of users opening source documents might indicate they are actively verifying the system's answers, while frequent "deep-dive" follow-up questions could suggest that the initial answers lacked sufficient detail.
...
The final concept proposed under service quality is "Knowledge Gap Detection," which aims to identify areas where the system or users may lack adequate knowledge. This feature builds upon the topic analysis discussed earlier (Section \ref{sec:topic_maps}) by leveraging the detected topics from user queries, retrieved documents, and knowledge assets. The goal is to highlight two distinct types of knowledge gaps:

\begin{enumerate}
    \item \textbf{Knowledge gap within the system:} This occurs when the topic of a user's question aligns with the topic of a relevant knowledge asset, but the documents actually retrieved by the system for that query do not adequately fit the topic. This suggests an internal deficiency in the system's ability to retrieve the correct information, despite the knowledge theoretically existing within its accessible sources. This could be due to issues with indexing, retrieval algorithms, or the structuring of the knowledge assets themselves.
...
\begin{enumerate}
    \item Implementation of a central data structure
    \item Implementation of a caching logic
    \item Topic maps
...
\subsection{Task type maps}
\subsection{Quality of the retrieval and the generated responses}
\subsection{Combining quality with task types and topics}
