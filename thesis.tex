...
ChatGPT reached 1 million users within 5 days after its launch in 2022. With this, it became the fastest-growing application in history and provides benefits for 800 million weekly users today. GenAI not only bares high potential for the private sector but also for the modern business world. Projections underscore this, suggesting that GenAI could increase Germany's GDP by up to \EUR{585} billion by 2040. Furthermore For enterprise applications, \textbf{Retrieval-Augmented Generation (RAG)} has emerged as a particularly suitable technology by connecting large language models (LLMs) to company-specific proprietary knowledge bases. Connecting and bounding LLMs to the proprietary knowledge bases not only lets them access up-to-date company data but also improves transparency and avoids hallucinations since the company data functions as the ground truth.

However, the success of enterprise RAG applications is not guaranteed and highly depends on the data the RAG system uses. RAG systems can be unstable, leading to low-quality results. Common failure points include the system failing to provide an answer even though the information is available, generating a false answer by misinterpreting context or returning an incomplete response. As organizations invest in and deploy these powerful but complex systems, a critical management challenge arises: How can the success of a GenAI application be measured and monitored over time? Without systematic evaluation, system administrators lack the data-driven insights needed to identify weaknesses, justify improvements, and ensure that the system delivers a positive return on investment.

This thesis addresses this management challenge directly. It documents the design, development, and evaluation of an analytics dashboard created to automatically measure the success of an enterprise GenAI application. Following the information systems success model by DeLone and McLean, success can be measured using three quality metrics - the information quality, the system quality and the service quality. However most RAG evaluation frameworks like RAGAS or ARES only include the information quality provided by the system. To move beyond this narrow perspective on IS success, this study employs a Design Science Research methodology to conceptualize and implement a dashboard grounded in a holistic view.
%---------------------------------------------------------------------%
